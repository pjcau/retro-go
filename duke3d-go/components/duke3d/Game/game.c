...
void findGRPToUse(char * groupfilefullpath){
    /* The launcher passes the chosen .grp as the ROM; its folder holds the
     * other data files (CON, RTS, DMO...). */
    const char *rom = rg_system_get_app()->romPath;
    if (rom && *rom)
    {
        strncpy(groupfilefullpath, rom, 511);
        strncpy(game_dir, rom, 511);
