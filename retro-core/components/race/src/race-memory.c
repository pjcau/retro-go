...
/* esp32-emu-turbo: 224 KB and 4 MB do not fit internal SRAM; PSRAM .bss (CONFIG_SPIRAM_ALLOW_BSS_SEG_EXTERNAL_MEMORY) */
#include <esp_attr.h>
EXT_RAM_BSS_ATTR unsigned char __attribute__ ((__aligned__(4))) mainram[(64+32+128)*1024];
/* ROM area for roms (4 Megabyte) */
EXT_RAM_BSS_ATTR unsigned char __attribute__ ((__aligned__(4))) mainrom[MAINROM_SIZE_MAX];
/* CPU internal ROM including vector table starting at 0xff0000 */
EXT_RAM_BSS_ATTR unsigned char __attribute__ ((__aligned__(4))) cpurom[256*1024];//prob only needs 0x10000
