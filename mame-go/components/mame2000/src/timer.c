...
#ifdef MAMEGO
/* mame-go save state (see mamego_state_walk() in cpuintrf.c): the scheduler
 * decides how many cycles each CPU runs before the next interrupt, so without
 * it a reloaded game drifts (Pac-Man's ghosts read the Z80 R register as
 * their random source). Entries are stored by index, their links as indices;
 * callbacks are raw pointers, valid because a state only loads on the build
 * that wrote it. Called at the frame boundary (no callback running). */
#define TIMER_IDX(p) ((p) ? (int)((p) - timers) : -1)
#define TIMER_PTR(i) ((i) >= 0 && (i) < MAX_TIMERS ? &timers[i] : NULL)
#define CPU_IDX(p)   ((p) ? (int)((p) - cpudata) : -1)
#define CPU_PTR(i)   ((i) >= 0 && (i) <= MAX_CPU ? &cpudata[i] : NULL)

size_t mamego_timer_state(unsigned char *buf, size_t size, int mode) /* 0 size, 1 save, 2 load */
{
	size_t pos = 0;
	int i, idx[6];

#define TS_IO(ptr, len) do { \
		size_t _l = (len); \
	...
	for (i = 0; i < MAX_TIMERS; i++)
	{
		...
		int links[2] = { TIMER_IDX(t->next), TIMER_IDX(t->prev) };
		TS_IO(links, sizeof(links));
		TS_IO(&t->callback, sizeof(t->callback));
		TS_IO(&t->callback_param, sizeof(t->callback_param));
		TS_IO(&t->enabled, sizeof(t->enabled));
		TS_IO(&t->period, sizeof(t->period));
		TS_IO(&t->start, sizeof(t->start));
		TS_IO(&t->expire, sizeof(t->expire));
		if (mode == 2)
		{
			t->next = TIMER_PTR(links[0]);
			t->prev = TIMER_PTR(links[1]);
		}
	}
	for (i = 0; i <= MAX_CPU; i++)
	{
		cpu_entry *c = &cpudata[i];
		TS_IO(&c->suspended, sizeof(c->suspended));
		TS_IO(&c->trigger, sizeof(c->trigger));
		TS_IO(&c->nocount, sizeof(c->nocount));
		TS_IO(&c->lost, sizeof(c->lost));
		TS_IO(&c->time, sizeof(c->time));
	}
	idx[0] = TIMER_IDX(timer_head);
	idx[1] = TIMER_IDX(timer_free_head);
	idx[2] = CPU_IDX(lastcpu);
	idx[3] = CPU_IDX(activecpu);
	idx[4] = CPU_IDX(last_activecpu);
	idx[5] = TIMER_IDX(callback_timer);
	TS_IO(idx, sizeof(idx));
	TS_IO(&base_time, sizeof(base_time));
	TS_IO(&global_offset, sizeof(global_offset));
	TS_IO(&callback_timer_modified, sizeof(callback_timer_modified));
	if (mode == 2)
	{
		timer_head = TIMER_PTR(idx[0]);
		timer_free_head = TIMER_PTR(idx[1]);
		lastcpu = CPU_PTR(idx[2]);
		activecpu = CPU_PTR(idx[3]);
		last_activecpu = CPU_PTR(idx[4]);
		callback_timer = TIMER_PTR(idx[5]);
	}
#undef TS_IO
	return pos;
}
