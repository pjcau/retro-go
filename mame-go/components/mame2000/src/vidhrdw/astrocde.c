...
#ifdef MAMEGO
	/* Fast path (Robby Roto, Wizard of Wor without stars): no sparkle on
	 * this line and an unrotated 8-bit bitmap -> the 8 pens of the line are
	 * fixed, write them straight into the row instead of plot_pixel() per
	 * pixel (and skip the 64-bit RNG offset only sparkle needs). */
	if (!(sparkle[line][0] | sparkle[line][1] | sparkle[line][2] | sparkle[line][3]) &&
		Machine->orientation == 0 && bitmap->depth == 8)
	{
		unsigned char pens[8], *dst = bitmap->line[line];
		int split = colorsplit[line];
		for (i = 0; i < 8; i++)
			pens[i] = Machine->pens[colors[line][i]];
		memloc = line * 80;
		for (i = 0; i < 80; i++, memloc++)
		{
			const unsigned char *p = pens + (i < split ? 4 : 0);
			data = (line < VerticalBlank) ? wow_videoram[memloc] : BackgroundData;
			dst[i*4+3] = p[data & 3];
			dst[i*4+2] = p[(data >> 2) & 3];
			dst[i*4+1] = p[(data >> 4) & 3];
			dst[i*4+0] = p[(data >> 6) & 3];
		}
		return;
	}
#endif
