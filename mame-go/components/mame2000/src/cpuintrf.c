...
/***************************************************************************
  ...
  mame-go: generic save state for simple 8-bit boards.

  MAME 0.37 can only save CPU registers, so mame-go snapshots what these
  boards keep between frames: the CPU contexts, the interrupt/scheduling
  counters and the CPU memory regions (which hold the work, video and sound
  RAM on these boards), plus the timer scheduler (timer.c). It is taken at
  the frame boundary, after cpu_run_step() has returned. Driver statics and
  sound-chip internals are not saved; the games this targets rewrite them
  every frame. Returns the state size (0 on error), or with mode 0 just the size.
...
#define MG_IO(ptr, len) do { \
...

	{
		 ...
		 * carries a fingerprint of the binary and a load from another build
		 * is refused instead of jumping into the void. */
		unsigned magic = MAMEGO_STATE_MAGIC;
		unsigned fingerprint = (unsigned)(uintptr_t)&mamego_state_walk;
		char name[16] = {0};
		strncpy(name, Machine->gamedrv->name, sizeof(name) - 1);
		if (mode == 2)
		{
			/* validate the header before touching anything */
			if (size < 24 || memcmp(buf, &magic, 4) || memcmp(buf + 4, &fingerprint, 4) || memcmp(buf + 8, name, 16))
				return 0;
			pos = 24;
		}
		else
	...
	for (i = 0; i < totalcpu; i++)
	{
		int len = GETCONTEXT(i, NULL);
		if (mode && pos + len > size) return 0;
		if (mode == 1)
		{
			if (cpu[i].save_context) memcpy(buf + pos, cpu[i].context, len);
			else GETCONTEXT(i, buf + pos);
		}
		else if (mode == 2)
		{
			if (cpu[i].save_context) memcpy(cpu[i].context, buf + pos, len);
			else SETCONTEXT(i, buf + pos);
			if (!cpu[i].save_context) SETIRQCALLBACK(i, cpu_irq_callbacks[i]);
		}
		pos += len;
		MG_IO(&cpu[i].iloops, sizeof(int));
		MG_IO(&cpu[i].totalcycles, sizeof(int));
		MG_IO(&cpu[i].vblankint_countdown, sizeof(int));
	}
	MG_IO(interrupt_enable, sizeof(interrupt_enable));
	MG_IO(interrupt_vector, sizeof(interrupt_vector));
	MG_IO(irq_line_state, sizeof(irq_line_state));
	MG_IO(irq_line_vector, sizeof(irq_line_vector));
	MG_IO(&watchdog_counter, sizeof(watchdog_counter));
	MG_IO(&vblank_countdown, sizeof(vblank_countdown));
	MG_IO(&current_frame, sizeof(current_frame));

	{
		extern size_t mamego_timer_state(unsigned char *buf, size_t size, int mode);
		size_t len = mamego_timer_state(NULL, 0, 0);
		if (mode && (pos + len > size || !mamego_timer_state(buf + pos, len, mode))) return 0;
		pos += len;
	}
...
{
	return mamego_state_walk(NULL, 0, 0);
}

int mamego_state_save(void *data, size_t size)
{
	return size >= mamego_state_size() && mamego_state_walk(data, size, 1) != 0;
}

int mamego_state_load(const void *data, size_t size)
{
	/* the walk validates the header and the total size before writing */
	if (size != mamego_state_size())
		return 0;
	return mamego_state_walk((unsigned char *)data, size, 2) != 0;
}
#endif
