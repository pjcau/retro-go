...
#ifdef MAMEGO
/* mame-go: the Euro revision (program ROMs bme01-03) that later MAME sets
 * ship as "1943"; 0.37b5 only knows the US and Japanese ones. Same board,
 * the other 34 ROMs are identical. */
ROM_START( 1943e )
	ROM_REGION( 0x30000, REGION_CPU1 )	/* 64k for code + 128k for the banked ROMs images */
	ROM_LOAD( "bme01.12d",   0x00000, 0x08000, 0x55fd447e )
	ROM_LOAD( "bme02.13d",   0x10000, 0x10000, 0x073fc57c )
	ROM_LOAD( "bme03.14d",   0x20000, 0x10000, 0x835822c2 )
...
#ifdef MAMEGO
GAMEX( 1987, 1943e,    1943,     1943,    1943,    0, ROT270, "Capcom", "1943 - The Battle of Midway (Euro)", GAME_NO_COCKTAIL )
#endif
