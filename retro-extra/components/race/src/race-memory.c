...
/* esp32-emu-turbo: allocated from PSRAM by race_alloc_memory() (main_ngp.c) so the
 * 4.4 MB is only taken while the NGP core runs, not by every retro-core app. */
...
unsigned char *cpurom; /* 256 KB, prob only needs 0x10000 */
